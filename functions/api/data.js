...
export async function onRequestGet(context) {
  ...
  const [prat, corr, doc, tl, prev, agg, med] = await Promise.all([
  ...
  const corrMap = {}, docMap = {}, tlMap = {}, prevMap = {}, aggMap = {}, medMap = {};
  ...
  const pratiche = (prat.results || []).map(p =>
    praticaRowToApp(p, corrMap[p.id] || [], docMap[p.id] || [], tlMap[p.id] || [], prevMap[p.id] || [], aggMap[p.id] || [], medMap[p.id] || [])
  );
